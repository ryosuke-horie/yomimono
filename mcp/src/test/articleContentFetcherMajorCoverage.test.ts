...
describe("ArticleContentFetcher 重要未カバー部分テスト", () => {
	describe("サイト固有抽出戦略の詳細テスト", () => {
		...
		it("Qiitaサイトの記事抽出戦略", async () => {
			...
			(mockPage.url as any).mockReturnValue(qiitaUrl);
			...
			// セレクター戦略のテスト
			Object.entries(qiitaSelectors).forEach(([key, selector]) => {
		...
		it("Zennサイトの記事抽出戦略", async () => {
			...
			(mockPage.url as any).mockReturnValue(zennUrl);
			...
			// セレクター検証
			Object.values(zennSelectors).forEach(selector => {
		...
		it("noteサイトの記事抽出戦略", async () => {
			...
			(mockPage.url as any).mockReturnValue(noteUrl);
		...
		it("Mediumサイトの記事抽出戦略", async () => {
			...
			(mockPage.url as any).mockReturnValue(mediumUrl);
			...
			const mediumSelectors = {
				...
				publishedDate: ".postMetaInline-authorLockup time, [data-testid='storyPublishDate']",
		...
		it("Dev.toサイトの記事抽出戦略", async () => {
			...
			(mockPage.url as any).mockReturnValue(devToUrl);
	...
	describe("高度なメタデータ抽出パターン", () => {
		it("JSON-LD構造化データの解析", () => {
			...
			// JSON-LD解析のパターンテスト
			const extractFromJsonLd = (jsonLd: any) => {
				const result: any = {};
	...
	describe("品質スコア計算の詳細テスト", () => {
		...
		it("抽出方法による信頼性スコア", () => {
			const calculateReliabilityScore = (extractionMethod: string, selectors: string[]) => {
			...
			expect(calculateReliabilityScore("structured-data", ["json-ld"])).toBeCloseTo(1.0, 1);
			expect(calculateReliabilityScore("site-specific", ["title", "content", "author"])).toBeCloseTo(0.95, 1);
	...
	describe("エラーハンドリングとフォールバック戦略", () => {
		it("セレクター失敗時のフォールバック", () => {
			const extractWithFallback = (selectors: string[], mockResults: (string | null)[]) => {
				for (let i = 0; i < selectors.length; i++) {
					const result = mockResults[i];
					if (result && result.trim()) {
						return { value: result, usedSelector: selectors[i], fallbackLevel: i };
					}
				}
	...
	describe("サイト別設定の詳細", () => {
		it("サイト固有の設定とオーバーライド", () => {
			...
			const getSiteConfig = (url: string) => {
				...
				return siteConfigs[domain as keyof typeof siteConfigs] || {
					selectors: {
						title: "h1",
...
if (import.meta.vitest) {
	...
	test("読み時間の計算", () => {
		const calculateReadingTime = (content: string, wordsPerMinute: number = 200) => {
			...
			const englishWords = content.split(/\s+/).filter(word => 
				/[a-zA-Z]/.test(word) && word.length > 0
			).length;
		...
		// 英語コンテンツ
		const englishContent = "This is a test article with multiple words for reading time calculation.";
		expect(calculateReadingTime(englishContent)).toBe(1);

		// 日本語コンテンツ
		const japaneseContent = "これは読み時間を計算するためのテスト記事です。".repeat(100);
		const japaneseTime = calculateReadingTime(japaneseContent);
		expect(japaneseTime).toBeGreaterThan(1);

		// 混在コンテンツ
		const mixedContent = "This is a mixed content article. これは混在コンテンツの記事です。".repeat(50);
	...
	test("URLパターンマッチング", () => {
		...
		// パターンマッチングのテスト
		expect(sitePatterns.qiita.test("https://qiita.com/user/items/abcd1234")).toBe(true);
		expect(sitePatterns.zenn.test("https://zenn.dev/user/articles/article-id")).toBe(true);
		expect(sitePatterns.note.test("https://note.com/user/n/n123456")).toBe(true);
		expect(sitePatterns.medium.test("https://medium.com/@user/article-title")).toBe(true);
		expect(sitePatterns.devTo.test("https://dev.to/user/article-title-123")).toBe(true);

		// 無効なURLのテスト
		expect(sitePatterns.qiita.test("https://qiita.com/invalid")).toBe(false);
		expect(sitePatterns.zenn.test("https://zenn.dev/user/books/book-id")).toBe(false);
	});
