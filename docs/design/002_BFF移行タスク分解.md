...
- [ ] Phase 3: フロントエンド呼び出しの BFF 置換
  ...
  - [ ] 差し替え時に各フローのテスト（コントラクト/ユニット/統合）を同時に追加し、不足が見えた部分のみ Phase4 で補完する運用にする。
- [ ] Phase 4: 品質保証と運用
  - [ ] Phase3 での差し替え時に書き切れなかったテストを補完（生成型との整合性チェックを含むコントラクトテストを優先）。
  - [ ] 生成した型ファイルを直接編集した差分を弾く独自のLint/CIルールを検討し導入（生成物の改変を検知して失敗させる）。
  - [ ] 運用手順のドキュメント化（`pnpm run generate`→`pnpm run orval`→`pnpm run lint`/`test` の実行順や、生成物コミットポリシー）。
