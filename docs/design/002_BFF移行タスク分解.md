...
- [ ] Phase 2: BFF (Route Handler) 実装基盤
  ...
  - [ ] BFF での共通関数を整備（エラー正規化とステータスコードのマッピングを優先対応。認証ヘッダーやログ/メトリクスは一旦スコープ外）。
  - [ ] キャッシュ/再検証方針を決定（Route Handler の応答に `Cache-Control` を付与するなど簡易運用で可。`next.revalidate`/`cache: 'no-store'` も状況に応じて選択）。
  - [ ] `.env` の取り扱いルールを明示し、外部 API キーがクライアントへ流出しない構成を徹底。
