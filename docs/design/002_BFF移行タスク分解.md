...
- Title: BFF基盤（Route Handler）実装  
  Description: Orvalのサーバー向け生成物を用いたBFFクライアントを用意し、エラー正規化とステータスコードマッピングを共通化。簡易キャッシュ方針（Cache-Control等）とAPIキー非露出の.env運用を整理する。
- Title: フロントAPI呼び出しのBFF差し替え  
  Description: 主要フロー（一覧/お気に入り/ラベルなど）を順次Route Handler経由に切り替え、OpenAPI生成型へ参照を寄せる。差し替えと同時に各フローのテスト（コントラクト/ユニット/統合）を追加し、旧クライアントや重複型を整理する。
- Title: テスト補完と生成型改変検知  
  Description: Phase3で漏れたテストを補完し、生成した型ファイルの直接編集を弾くLint/CIルールを導入する（生成物改変を検知して失敗させる）。
