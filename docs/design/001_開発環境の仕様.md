...
- CLI エントリーポイント `src/scripts/seed-runner.ts` は `SEED_PRESET` のみを受け付けます。`default`（または未設定）で生成、`clear` で削除のみを実行します。カスタム件数指定はサポートしていません。
...
3. クリアのみ必要な場合は `NODE_ENV=development SEED_PRESET=clear pnpm exec tsx src/scripts/seed-runner.ts`

## 6. よくあるエラーと対処

- `no such table: bookmarks` → ローカル D1 にマイグレーションが未適用。`pnpm run migrate:dev:local` を再実行。
- `シードデータ実行はサポートされていません` → `NODE_ENV=production` で Seed を実行している。`NODE_ENV=development` に戻す。
