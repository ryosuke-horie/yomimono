...
> Seedコマンドは `.wrangler/state/v3/d1` にある Miniflare D1 ファイルを直接操作します。クリアのみ実行したい場合は `NODE_ENV=development SEED_PRESET=clear pnpm exec tsx src/scripts/seed-runner.ts` を直接実行してください。
