...
| コマンド | 内容 | 補足 |
| --- | --- | --- |
| `pnpm run seed` | デフォルト設定（25件/6ラベル/30%お気に入り）で Miniflare D1 に投入 | コマンド内でクリア→投入を実行 |
...
2. **Seed投入**: `pnpm run seed`（内部でクリア→投入）または `seed:test`
...
> Seedコマンドは `.wrangler/state/v3/d1` にある Miniflare D1 ファイルを直接操作します。複数のD1ファイルがある場合は `D1_SQLITE_PATH` で対象パスを指定できます。クリアのみ実行したい場合は `NODE_ENV=development SEED_PRESET=clear pnpm exec tsx src/scripts/seed-runner.ts` を直接実行してください。
