...
このドキュメントでは、API と Frontend のローカル開発・テスト手順を1か所に集約します。日常的な操作はここを参照し、`api/README.md` は本番デプロイ手順に専念させます。
...
- ロジックは `src/db/seed/seed.ts` に集約されており、`runSeedData` がブックマーク/ラベル/お気に入りをまとめて生成、`clearDatabase` が既存レコードを削除します。
- CLI エントリーポイント `src/db/seed/seed-runner.ts` は `SEED_PRESET` に `clear` を指定した場合のみ削除を実行し、それ以外は常にデフォルト件数で投入します。
...
## 7. Frontend: セットアップ & スクリプト

### 7.1 セットアップ手順

1. `cd frontend`
2. 依存インストール: `pnpm install`
3. `.env.local` を作成（`NEXT_PUBLIC_API_BASE_URL=http://localhost:8787` など）
4. ローカル起動: `pnpm run dev`（Next.js デフォルトでポート3000）

### 7.2 package.json スクリプト概要

| コマンド | 説明 | 備考 |
| --- | --- | --- |
| `pnpm run dev` | Next.js 開発サーバー起動 | API をローカル(8787)に向ける |
| `pnpm run build` / `start` | 本番ビルド / サーバー起動 | Lefthookのpre-pushでも実行 |
| `pnpm run test` | Frontend のVitestを実行 | `vitest.config.ts` を参照 |
| `pnpm run lint` | Biomeで静的解析 | `pnpm run format` で整形 |

### 7.3 API との連携

- Frontendテスト/開発時は API を `pnpm run dev` で起動し、`.env.local` で `http://localhost:8787` を参照させる。
- E2E/統合テストを行う場合は、Seed (`pnpm run seed`) によって初期データを投入した状態で API を起動してから Frontend を実行する。
