...
## 段階的タスク
- [x] Phase 1: OpenAPI 生成と型配布のパイプライン確立
  - [x] `api/openapi.yaml` を単一ソースとして生成するコマンドを整備（`pnpm run generate` を標準とする）。生成元/責務分離を README/ADR に明記し、ドキュメント作成自体は別 Issue で管理。
  - [x] Orval 設定で API/BFF/フロント向けの生成先を決定し、モノレポ内の依存関係を固定（例: `api` → `frontend/src/lib/openapi` へ `pnpm run orval`）。
  - [x] CI で OpenAPI/Orval の乖離検知（未コミット生成物の差分チェック）を組み込み、手元での実行手順を docs に追加。
- [x] Phase 2: BFF (Route Handler) 実装基盤
  - [x] Next.js Route Handler 向けの BFF クライアントを Orval 生成に含め、サーバー専用クライアントとフロント用クライアントのビルド設定を分離。
  - [x] BFF での共通関数を整備（エラー正規化とステータスコードのマッピングを優先対応。認証ヘッダーやログ/メトリクスは一旦スコープ外）。
  - [x] キャッシュ/再検証方針を決定（Route Handler の応答に `Cache-Control` を付与するなど簡易運用で可。`next.revalidate`/`cache: 'no-store'` も状況に応じて選択）。
  - [x] `.env` の取り扱いルールを明示し、外部 API キーがクライアントへ流出しない構成を徹底。
- [x] Phase 3: フロントエンド呼び出しの BFF 置換
  - [x] 影響範囲の高いフローから順次 BFF 経由に差し替え（クエリ hooks/サーバーアクション/SSR などパターン別にガイド化）。
  - [x] 型の参照元を OpenAPI 生成物に寄せ、既存の重複型を整理。不要になったクライアント実装を段階的に除去。
  - [x] BFF での正規化レスポンスに合わせた UI 側のエラーハンドリング/ローディング表現を更新。
  - [x] 差し替え時に各フローのテスト（コントラクト/ユニット/統合）を同時に追加し、不足が見えた部分のみ Phase4 で補完する運用にする。
- [x] Phase 4: 品質保証と運用
  - [x] Phase3 での差し替え時に書き切れなかったテストを補完（生成型との整合性チェックを含むコントラクトテストを優先）。
  - [x] 生成した型ファイルを直接編集した差分を弾く独自のLint/CIルールを検討し導入（生成物の改変を検知して失敗させる）。

## アウトプット（Issue候補: Title / Description セット）
- Title: OpenAPI生成パイプライン整備  
  Description: `pnpm run generate` で `api/openapi.yaml` を生成する唯一のソースを確立し、Orval でブラウザ/BFF/必要ならAPI向けに型・クライアントを分岐生成できるよう設定する。生成物乖離をCIで検知する。
- Title: BFF基盤（共通部品）実装  
  Description: Orvalのサーバー向け生成物を用いたBFFクライアントを用意し、エラー正規化とステータスコードマッピングを共通化。簡易キャッシュ方針（Cache-Control等）とAPIキー非露出の.env運用を整理する。
- Title: BFF適用（ブックマーク系フロー）  
  Description: ブックマーク一覧/既読・未読切替/お気に入り/ラベル付与等のフローをRoute Handler経由に差し替え、OpenAPI生成型へ参照を寄せる。差し替えと同時に該当フローのテスト（コントラクト/ユニット/統合）を追加し、旧クライアントや重複型を整理する。
- Title: BFF適用（ラベル系フロー）  
  Description: ラベル一覧/作成/更新/削除/クリーンアップ等のフローをRoute Handler経由に差し替え、OpenAPI生成型へ参照を寄せる。差し替えと同時に該当フローのテストを追加し、旧クライアントや重複型を整理する。
- Title: テスト補完（生成型整合性含む）  
  Description: BFF差し替え時に漏れたテストを補完し、生成型との整合性チェックを含むコントラクトテストを追加する。
- Title: 生成型改変検知ルール導入  
  Description: 生成した型ファイルを直接編集した差分を弾く独自のLint/CIルールを導入し、生成物改変を検知して失敗させる。

## 未決定事項・検討ポイント
- OpenAPI 生成のソース: 手書き `api/openapi.yaml` を正として維持し、そこをベースに生成する（Honoスキーマ自動出力は併用しない方針）。
- Orval の生成ターゲット: フロントエンドの共有 lib としてまとめ、サーバー/ブラウザ向けの生成物を同居させる（frontend 配下で完結）。
- キャッシュ戦略: キャッシュが必須なアプリではないため、運用が楽な最小構成（基本は `cache: 'no-store'`/非ISR、必要に応じて軽微なCache-Control付与程度）。
...
1. OpenAPI→Orval→多層生成
   - Hono 側にスキーマを付け、`pnpm run generate` で `api/openapi.yaml` を生成するのを唯一のソースにする。
