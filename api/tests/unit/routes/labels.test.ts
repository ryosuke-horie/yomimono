...
function compareObjectsIgnoringDateFormat(actual: any, expected: any): void {
...
function createMockLabelsRouter() {
  ...
  router.post("/", async (c) => {
    try {
      ...
      const newLabel = await mockLabelService.createLabel(labelName, description);
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const updatedLabel = await mockLabelService.updateLabelDescription(id, body.description);
...
describe("Labels Route", () => {
  ...
  describe("PATCH /api/labels/:id", () => {
    ...
    it("ラベルの説明文を更新し、成功レスポンスを返すこと", async () => {
      ...
      expect(body.success).toBe(true);
      compareObjectsIgnoringDateFormat(body.label, mockLabel);
      expect(mockUpdateLabelDescription).toHaveBeenCalledWith(labelId, newDescription);
    });
