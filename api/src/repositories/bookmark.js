...
export class DrizzleBookmarkRepository {
	...
	async attachLabelAndFavoriteStatus(where) {
		const query = this.db
			.select({
				bookmark: bookmarks,
				favorite: favorites,
	...
	async addToFavorites(bookmarkId) {
		try {
			// ブックマークの存在確認
			const bookmark = await this.db
				.select()
	...
	async removeFromFavorites(bookmarkId) {
		try {
			const result = await this.db
				.delete(favorites)
				.where(eq(favorites.bookmarkId, bookmarkId))
	...
	async getFavoriteBookmarks(offset, limit) {
		try {
	...
	async findByLabelName(labelName) {
		try {
			const results = await this.db
				.select({
					bookmark: bookmarks,
				...
				.leftJoin(favorites, eq(bookmarks.id, favorites.bookmarkId))
				.where(and(eq(labels.name, labelName), eq(bookmarks.isRead, false)))
				.all();
			return results.map((row) => ({
				...row.bookmark,
				isFavorite: !!row.favorite,
	 ...
	 */
	async findWithoutSummary(limit = 10, orderBy = "createdAt", offset = 0) {
		try {
			const query = this.db
				.select({
					bookmark: bookmarks,
			...
			const sortedQuery =
				orderBy === "readAt"
					? query.orderBy(bookmarks.updatedAt)
					: query.orderBy(bookmarks.createdAt);
			const results = await sortedQuery.all();
			return results.map((row) => {
				const bookmark = row.bookmark;
				return {
					...bookmark,
			...
			console.error("Failed to find bookmarks without summary:", error);
			throw error;
		}
	}
	/**
	...
	async updateSummary(id, summary) {
		try {
			...
			await this.db
				...
				.set({
					summary,
					summaryCreatedAt: bookmark.summary ? bookmark.summaryCreatedAt : now,
