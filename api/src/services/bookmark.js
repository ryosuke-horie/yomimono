export class DefaultBookmarkService {
	repository;
	constructor(repository) {
		this.repository = repository;
	}
	async getUnreadBookmarksCount() {
		return await this.repository.countUnread();
	}
	async getTodayReadCount() {
		return await this.repository.countTodayRead();
	}
	async getUnreadBookmarks() {
		// Update return type
	...
	async createBookmarksFromData(bookmarks) {
		// 既存のブックマークをURLで検索
	...
	async markBookmarkAsRead(id) {
		const updated = await this.repository.markAsRead(id);
		if (!updated) {
			throw new Error("Bookmark not found");
		}
	}
	async getRecentlyReadBookmarks() {
		try {
	...
	async getBookmarksByLabel(labelName) {
		try {
